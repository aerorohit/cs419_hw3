...
class myNeuralNet:
	...
	def __init__(self, dim_input_data, dim_output_data): # you can add/modify arguments of this function 
		...
		inpl=tf.placeholder(tf.float32,[None,784])#
		lbpl=tf.placeholder(tf.float32,[None,5])#
	...
	def addHiddenLayer(self, layer_dim, activation_fn=None, regularizer_fn=None):
		...
		a=tf.layers.dense(inpl,units=layer_dim)#
		r=activation_fn(a)
	...
	def addFinalLayer(self, activation_fn=None, regularizer_fn=None):
		...
		y=tf.layers.dense(r,units=self.dim_output_data)#
		k=tf.nn.softmax_cross_entropy_with_logits_v2(labels=lbpl,logits=y)#
	...
	def setup_training(self, learn_rate):
		...
		self.loss=tf.reduce_mean(k)#
		# Define the train step as self.train_step = ..., use an optimizer from tf.train and call minimize(self.loss)
		self.train_step=tf.train.GradientDescentOptimizer(learn_rate).minimize(loss)#
		pass
	...
	def setup_metrics(self):
		...
		correct_prediction = tf.equal(tf.argmax(y, axis=1), tf.argmax(lbpl, 1))
